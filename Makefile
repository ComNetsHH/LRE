...
LRE : *.cc *.h
	$(CC) $(INCLUDE2.7) *.cc -o LRE $(LIBRARIES2.7)
	$(CC) $(INCLUDE2.7) *.cc -o LRE.so $(LIBRARIES2.7) -shared -fPIC

LRE3.5 : *.cc *.h
	$(CC) $(INCLUDE3.5) *.cc -o LRE $(LIBRARIES3.5)
	$(CC) $(INCLUDE3.5) *.cc -o LRE.so $(LIBRARIES3.5) -shared -fPIC

LRE3.6 : *.cc *.h
	$(CC) $(INCLUDE3.6) *.cc -o LRE $(LIBRARIES3.6)
	$(CC) $(INCLUDE3.6) *.cc -o LRE.so $(LIBRARIES3.6) -shared -fPIC
